  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="flex items-center gap-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-purple-500/50 px-4 py-2.5 rounded-xl text-sm font-bold transition-all"
      >
        <selected.icon size={16} className="text-purple-600" />
        <span className="text-zinc-700 dark:text-zinc-200">{SORT_LABELS[value]}</span>
        <ChevronDown size={14} className={`text-zinc-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xl z-50 overflow-hidden">
