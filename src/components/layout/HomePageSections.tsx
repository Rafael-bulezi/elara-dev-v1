      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base md:text-lg font-black text-zinc-900">Ofertas do Dia</h2>
    ...
    <div className="max-w-[1400px] mx-auto px-4 md:px-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {SHOWCASE_PODS.map((pod, podIdx) => {
          ...
          return (
            <div key={pod.cat} className={`rounded-xl md:rounded-2xl border p-3 md:p-4 flex flex-col gap-2.5 md:gap-3 ${podBg}`}>
              <div>
                <h3 className={`text-xs md:text-sm font-black leading-tight ${headClr}`}>{pod.title}</h3>
                <p className={`text-[10px] md:text-[11px] mt-0.5 hidden sm:block ${subClr}`}>{pod.sub}</p>
              </div>

              <div className="grid grid-cols-2 gap-1.5 md:gap-2">
                {(items.length >= 4 ? items : [...items, ...items, ...items, ...items]).slice(0, 4).map((p, i) => (
                  <div key={`${p.id}-${i}`} onClick={() => onProductClick(p)} className="cursor-pointer group">
                    <div className={`aspect-square rounded-lg md:rounded-xl overflow-hidden mb-1 ${imgBg}`}>
                      <img src={p.image} alt={p.title} loading="lazy"
                        className="w-full h-full object-cover group-hover:scale-[1.05] transition-transform duration-200" />
                    </div>
                    <p className={`text-[9px] md:text-[10px] line-clamp-1 font-medium leading-tight ${titleClr}`}>
              ...
              <button onClick={() => onSelectCategory(pod.cat)}
                className={`text-[10px] md:text-xs font-bold flex items-center gap-1 mt-auto pt-2 border-t ${divider} ${linkClr}`}>
                {pod.link} <ArrowRight size={11} />
              </button>
            </div>
          );
        })}
...
export const ImportCTA: React.FC<{ onOpenImport: () => void }> = ({ onOpenImport }) => (
  <section className="bg-[#5A189A] py-8 md:py-10 px-4">
    <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-5 md:gap-6">
      <div className="text-center sm:text-left">
        <p className="text-purple-300 text-xs font-black uppercase tracking-widest mb-2">Importação Personalizada</p>
        <h3 className="text-white font-black text-xl md:text-3xl leading-tight mb-2">
      ...
      <button onClick={onOpenImport}
        className="shrink-0 flex items-center gap-2 bg-white text-[#5A189A] px-6 md:px-7 py-3 md:py-3.5 rounded-xl font-black text-sm hover:bg-zinc-100 active:bg-zinc-200 transition-colors shadow-xl">
        <Globe size={18} />
        Pedir Importação
      </button>
    </div>
...
export const DuoGrid: React.FC<CommonProps & { categories: string[]; onSelectCategory: (name: string) => void }> = ({
  ...
  return (
    <section className="py-6 md:py-8 bg-zinc-50 border-b border-zinc-100">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Percent size={18} className="text-amber-500" />
            <h2 className="text-base md:text-lg font-black text-zinc-900">{cats.join(' & ')}</h2>
          </div>
...
export const StatsBanner: React.FC = () => (
  <section className="py-8 md:py-10 bg-zinc-900">
    <div className="max-w-[1400px] mx-auto px-4 md:px-8">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-6 text-center">
        {[
          ...
          <div key={label}>
            <p className="text-2xl md:text-4xl font-black text-white mb-1">{value}</p>
            <p className="text-xs md:text-sm text-zinc-400 font-semibold">{label}</p>
          </div>
        ))}
      </div>
    </div>
