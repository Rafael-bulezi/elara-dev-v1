...
const WishlistDrawer: React.FC<WishlistDrawerProps> = ({
  ...
  <>
    ...
    <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-zinc-950 z-[70] shadow-2xl flex flex-col transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-4 border-b border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center gap-2">
          <Heart size={18} className="text-rose-500 fill-rose-500" />
          <h2 className="font-black text-zinc-900 dark:text-white">Favoritos</h2>
          <span className="text-sm text-zinc-400">({items.length})</span>
        </div>
        <button onClick={onClose} className="p-1 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-900">
      ...
      <div className="flex-1 overflow-y-auto">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
            <Heart size={48} className="text-zinc-200 dark:text-zinc-700" />
            <p className="font-bold text-zinc-500 dark:text-zinc-400">A sua lista de favoritos está vazia</p>
            ...
            {items.map(item => (
              <div key={item.id} className="flex gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-zinc-900 dark:text-white line-clamp-2">{item.title}</p>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button onClick={() => onRemove(item.id)} className="p-1.5 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-rose-500">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
